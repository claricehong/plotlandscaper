---
title: Introduction to plotlandscaper
...
vignette: >
    %\VignetteIndexEntry{Introduction to plotlandscaper}
...
```
`plotlandscaper` is a coordinate-based, genomic visualization package for R. Using
`grid` graphics, `plotlandscaper` empowers users to programmatically and flexibly
generate multi-panel figures. Tailored for genomics for a variety of genomic
assemblies, `plotlandscaper` allows users to visualize large, complex genomic
datasets while providing exquisite control over the arrangement of plots.

`plotlandscaper` functions can be grouped into the following categories:

-   **Page layout functions:**

Functions for creating `plotlandscaper` page layouts, drawing, showing, and hiding
guides, as well as placing plots on the page. See [The plotlandscaper
Page](https://phanstiellab.github.io/plotlandscaper/articles/guides/plotgardener_page.html)

-   **Reading functions:**

Functions for quickly reading in large biological datasets. See [Reading Data
for plotlandscaper](https://phanstiellab.github.io/plotlandscaper/articles/guides/reading_data_for_plotgardener.html)
...
Data](https://phanstiellab.github.io/plotlandscaper/articles/guides/plotting_multiomic_data.html)

-   **Annotation functions:**

Enables users to add annotations to their plots, such as legends, axes, and
scales. See [Plot Annotations](https://phanstiellab.github.io/plotlandscaper/articles/guides/annotations.html)

-   **Meta functions:**

Functions that display `plotlandscaper` properties or operate on other
`plotlandscaper` functions, or constructors for `plotlandscaper` objects. See
[plotlandscaper Meta Functions](https://phanstiellab.github.io/plotlandscaper/articles/guides/plotgardener_meta_functions.html)

This vignette provides a quick start guide for utilizing `plotlandscaper`. For
in-depth demonstrations of `plotlandscaper`'s key features, see the additional
...
`plotlandscaper` plotting functions contain 4 types of arguments:
...
generate a `plotlandscaper` plot that fills up the entire graphics window and
cannot be annotated. **These plots are only meant to be used for quick**
**genomic data inspection and not as final `plotlandscaper` plots.** The only
...
# Plotting and annotating on the `plotlandscaper` page

To build complex, multi-panel `plotlandscaper` figures with annotations, we must:

1.  Create a `plotlandscaper` coordinate page with `pageCreate()`.
...
3.  Annotate `plotlandscaper` plot objects by passing them into the `plot`
...
`plotlandscaper` page, check out the section [Working with plot
objects](https://phanstiellab.github.io/plotlandscaper/articles/guides/plotgardener_page.html#working-with-plot-objects).

# Exporting plots

When a `plotlandscaper` plot is ready to be saved and exported, we can first
...
We still have many ideas to add for a second version of `plotlandscaper`
...
suggestions for ways we can improve `plotlandscaper`, please let us know!
